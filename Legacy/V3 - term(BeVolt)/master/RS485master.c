...
static unsigned char RS485_outChecksum;					//checksum to send 
static unsigned char RS485_inChecksum;						//checksum from slaves
static unsigned char RS485_inDataPointer;					//array element pointer when receiver DATA from slvaes
...
static unsigned char RS485_outDataPointer;					//array element pointer when sending DATA to slaves
static unsigned char RS485_goodPacket;						//slave sent a good packet
static unsigned char RS485_receive;						//flag telling it to wait for a new packet to come in or just transmit 1 = works normally, 0 = just transmit (testing)
					
//local functions
static void RS485_receiverReset(void);
static void RS485_transmitterReset(void);
...
void __attribute__((__interrupt__, auto_psv)) _U1RXInterrupt(void)
{
	...
	{											//this is the first byte received
		...
		{
		//	RS485_status = 6;					//i"m just going to ignore a bad start byte. seems the RS485 driver chip on the V3 boards likes to generate bullshit start bytes when transmitting???
