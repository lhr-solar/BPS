...
static uint16_t SPI_WriteRead(spi_port_t port, uint8_t txData){
    if(port >= NUM_SPI_BUSSES) return SPI_ERROR; //set first 8 bits to 1 to signal error

    SPI_Wait(port);
    SPI_BUSSES[port]->DR = txData & SPI_SUCCESS;
    SPI_Wait(port);
    return SPI_BUSSES[port]->DR & SPI_SUCCESS; //normal return will have the first 8 bits as 0
}
