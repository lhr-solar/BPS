...
#define CAN_FIFO_SIZE  32 //most messages it can take before wrapping around

static CanTxMsg TxMessage;
static CanRxMsg RxMessage;
static uint8_t rxFifo[CAN_FIFO_SIZE][8] = {0}; //initialize 32 by 8 array containing FIFO data 
...
uint8_t BSP_CAN_Read(uint32_t *id, uint8_t *data) {
    //if the head and tail are not the same
