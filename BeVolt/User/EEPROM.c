...
void EEPROM_Log(uint8_t logType, uint8_t data){
	uint8_t fault_stat = EEPROM_ReadByte(EEPROM_FAULT_CODE_ADDR);
	...
	EEPROM_WriteByte(EEPROM_FAULT_CODE_ADDR, fault_stat);	// Update the fault address
	
	EEPROM_WriteByte(logType + (data / 8), 1 << (data % 8)); // Update the more specific address.
}
