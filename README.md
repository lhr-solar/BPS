...
the simulator can be compiled with `make simulator`. The simulator also has an optional command line argument `TEST=` to specify the name of the test file to run. Note: when typing name it, don't type the `Test_` part or the `.c` part.
