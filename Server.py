...
#####Custom Libs#####
import ClientHandler #what clients are passed off too
...
class Server(threading.Thread): #inherits from threading so that you can run multiple servers on the same computer
    ...
    def __init__(self, host = "127.0.0.1", port= 5000, backlogs= 100, banFile= None):
        ...
        self.openFiles = [] #used for client connections to check and make sure they aren't wiring data to, or opening already open files
