...
static void SOC_firstRun(void);
static void SOC_writeACC(unsigned long int acc);
static unsigned char SOC_checksum1(unsigned long int in);
static unsigned char SOC_checksum2(unsigned long int in);
...
//local globals
static unsigned char SOC_timerOverflow;		//flag set if the timer overflows.
static unsigned char SOC_initialized;			//binary flag - checks if we're the first run or not.
...
//this updates the SOC accumulator global itself!
//since the compiler didn't seem to like to return a ulong.
//this is caled when the termianl forces a new SoC value
