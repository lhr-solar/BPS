...
static int LTC_waiting;			//flag for the timer
static int LTC_version;			//set if it is LTC6802 or LTC6803; 2 = LTC6802, 3 = LTC6803
static int LTC_verifyPEC;			//do we check the PEC on received data (for both types of chips)(0 = ignore PEC on receive, 1 = reject received data with bad PEC)
...
static void setPECbyte(char m);
static char LTC_calculatePEC2(char *data, unsigned int length);
static char LTC_checkPEC(char *data, char CRC, unsigned int length);
static void LTC_sendPoll(void);
static char LTC_calculatePEC(char *data, unsigned int length); //Calculate CRC byte for "length" bytes of data
static unsigned char LTC_SMBUS(unsigned char inPEC, char inByte);
static unsigned char LTC_calculatePEC3(char *data, unsigned int length);
...
{	//written by Archer
	//turns an 18-char array into a 12 element int array
	//values are 12 bits, so 3 bytes can hold 2 12bit ints
	int i;
	int j;
	int buffer;
	for(j=0, i=1; i < 17; )
...
static void init_crc8() {
 ...
 static    char LTC_calculatePEC2(char *data, unsigned int length) {            // np is number of bytes currently in packet[]
       int z;
