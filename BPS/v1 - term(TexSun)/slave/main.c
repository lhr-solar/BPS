...
unsigned char OWgood;			//1 = connections good, 0 = open sense wire
...
int main(void)
{
	...
	LTC_setConfig(0x91,0,0,0x80,125,175);
	//status = LTC_setConfig(0x11,0,0,0x80,125,175);
	tempGain[0] = 1;
	temp = LTC_calculatePEC(tempGain, 1);
	temp2 = LTC_calculatePEC2(tempGain, 1);
	i++;
	i++;
...
void initialize()
{
	...
	LTC_timerInitialize();	//configure the timer(that waits for the LTC's AD conversion)
