...
void ADC_initialize(void)
{
	...
	ADCON2 = 0x6428;			//use external Vref+, external Vref-, scan enabled, 16 word sinle buffer, use only mux A, and scan 10 inputs
...
void ADC_scale(void)
{
	ADC_currLow = ((ADC_currLow - 2048)*reg_read(5,1))/2 + reg_read(5,2);
	ADC_currHigh = (ADC_currHigh - 2048)*reg_read(5,3) + reg_read(5,4);
