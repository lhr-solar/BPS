...
waitingCMD enquedCMDs[MAX_ENQUED_CMDS];				//holds all of the waitingCMDs (currently 5)
unsigned char RS422_queIn;							
unsigned char RS422_queCount;
unsigned char RS422_inQueHandler;
...
extern unsigned char RS485_outChecksum;					//checksum to send 
extern unsigned char RS485_inChecksum;						//checksum from slaves
extern unsigned char RS485_outLength;						//DATA length on commands
extern unsigned char RS485_inLength;

//packet mode variables
...
unsigned char RS422_sending;						//1 if we are currently sending a packet, 0 otherwise

//terminal mode variables
unsigned char RS422_termMode;						//0 = normal (packet) mode, 1 = interactive terminal mode
unsigned char RS422_termEscapeCount;				//counts the number of escape characters received in sequence
unsigned char RS422_termOutPointer;					//counters the characters sent out
unsigned char RS422_termInPointer;					//counts the characters received
unsigned char RS422_termReceiving;					//0 = not receiving (waiting for user to type first character), 1 = user is typing (user has sent one command, but not pressed enter yet)
unsigned char RS422_termSending;					// 0 = not sending, 1 = in the process of sending
...
extern time RTC_getTime(void);		//gotta re-declare this function here, or otherwise it'll bitch, alot.
extern unsigned char TX_sendingBatt;
				
void RS422_processPacket(void);
void RS422_runQuedCMD(unsigned char item);
void RS422_sendPacket(void);
void RS422_initialize(void);
unsigned char RS422_ready(void);
void RS422_enqueCMD(void);
...
unsigned char RS422_termSending;					// 0 = not sending, 1 = in the process of sending
unsigned char RS422_termInBuff[255];				//holds one line of user input
unsigned char RS422_termutBuff[255];				//holds the output to the user
unsigned char RS422_termOutLength;	
	*/			
...
//unsigned char BPSstate;	//0 = system is "off" and not measuring any cells, contactor is off
							...
							//5 = system is commanded on, do a measure cycle and then  turn the contactor on 
void RS422_processPacket(void)
{
