...
#define FAKEVOLTS 33000
#define FAKETEMPS 28000
...
void __attribute__((__interrupt__, auto_psv)) _U1RXInterrupt(void)
{
	...
	else if (RS485_inStep == 2)
	{
		...
		if(temp <= 15 && misc_isDebug())	 	//respond to all valid slave addresses, only if the DEBUG switch is on. this is to make sure that this dummied out slave doesn't cause issues in a real system
		{										//yes it is for me
...
static void RS485_processPacket()
{
	...
	else if (RS485_command == 7)		//read voltages
	{
		if(RS485_inLength == 0)
		{
			...
			if(misc_isCalUnlocked())		//is cal dip swith is on, it always sends out 12 readings
				readno = 12;
			RS485_outLength = 2*readno;
			RS485_response = 0;
			for(i = 0; i < readno; i++)
			{
				RS485_outData[2*i]   = RS485_splitMSB(FAKEVOLTS);
				RS485_outData[2*i+1] = RS485_splitLSB(FAKEVOLTS);
			}
		}
		else
		{
	...
	else if (RS485_command == 8)		//read temp
	{
		if(RS485_inLength == 0)
		{
			...
			for(i = 0; i < readno; i++)
			{
				RS485_outData[2*i]   = RS485_splitMSB(FAKETEMPS);
				RS485_outData[2*i+1] = RS485_splitLSB(FAKETEMPS);
			}
		}
		else
		{
