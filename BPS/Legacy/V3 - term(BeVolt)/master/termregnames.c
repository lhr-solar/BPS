	...
	if(type == 0)		//master
	{
		if(group == 1)	//CONFIG group
		{
			if(reg == 1)
				...
				term_appendString("1.11: \"IGNITION\" input enable (0 = serial commands, 1 = use hardware ign input(strict), 2 = hardware ign (with termianl override)."); 
			else if(reg == 12)
				term_appendString("1.12: number of SOC packets per update cycle."); 
			else if(reg == 13)
				term_appendString("1.13: board hardware version (1 = v1 (Solorean), 2  = v2 (TexSun), 3 = v3 (new car)."); 
			else if(reg == 14)
				term_appendString("1.14: precharge enable (0 = no precharge, 1 = precharge)"); 
			else if(reg == 15)
				term_appendString("1.15: precharge contactor mode (0 = normal)"); 
			else if(reg == 16)
				term_appendString("1.16: aux contactor mode (0 = 0 = always off, 1 = follow main contactor with precharge logic, 2 = strictly mirror main contactor, 3 - follow main contactor with prechg, but keep on during trip, 4 - always on"); 
			else if(reg == 17)
				term_appendString("1.17: aux to precharge time (units of 0.1 second)"); 
			else if(reg == 18)
				term_appendString("1.18: precharge to main contactor time (units of 0.1 second)"); 
			else if(reg == 19)
				term_appendString("1.19: precharge time after main is on (units of 0.1 second)"); 
			return;
		}
		...
		else if(group == 6)		//CAN BUS
		{
			if(reg == 1)
				term_appendString("6.1: CAN base address. (0-15)(sets the high 4 bits of MSG ID)");
			else if(reg == 2)
				term_appendString("6.2: CAN RX enable (0 = disabled, 1 = BPS responds to CAN commands)");
			else if(reg == 3)
				term_appendString("6.3: CAN TX enable (0 = disabled, 1 = BPS sends CAN updates)");
			return;
		}
		else if(group == 7)		//LCD display
		{
			if(reg == 1)
				term_appendString("7.1: LCD_period: # of main loop iterations between LCD updates (0 = LCD is off)"); 
			return;
		}	
		return;
	}
