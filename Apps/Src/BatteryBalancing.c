...
static void BattBalancing_ClearDischargeBit(int Cell, uint8_t total_ic, cell_asic *ic);
static void BattBalancing_GetICNumber(uint8_t i, uint8_t* ICNumber, uint8_t* ModuleNumber);
static void BattBalancing_GetICNumber(uint8_t i, uint8_t* ICNumber, uint8_t* ModuleNumber);
static void BattBalancing_SetDischargeBit(uint8_t module, cell_asic ic[]);
...
void BattBalancing_Balance(cell_asic Minions[]){ 
	...
	for (uint8_t k = 0; k < NUM_BATTERY_MODULES; k++) {
		...
		if (voltage > lowest + BALANCING_TOLERANCE_START) {	
			BattBalancing_SetDischargeBit(k, Minions);	
		}
		//Clear discharge bit of module if it reaches minimum
		else if(voltage < lowest + BALANCING_TOLERANCE_STOP) {
			uint8_t ICIndex;
			uint8_t MNumber;
			BattBalancing_GetICNumber(k, &ICIndex, &MNumber);	
			BattBalancing_ClearDischargeBit(MNumber, NUM_MINIONS, &Minions[ICIndex]);	
		}
	}
}
